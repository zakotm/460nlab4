...
if(GetIRD(CURRENT_LATCHES.MICROINSTRUCTION)==1) /*check IRD*/
	...
	else 
	{
		...
		{ /*cases represent what the COND1 and COND0 responses are 3 = addr mode, r = priv mode, 5 = interrupt list/test for interrutps*/
		case 0:
			break;
		case 1:
		...
		case 3:
			...
			break;
		case 4:
			temp = ((temp) | ((CURRENT_LATCHES.PSR & 0x8000)>>12));  /*15-3 = 12 right shift checking for user privilege*/
		break;
	...
	if(GetR_W(CURRENT_LATCHES.MICROINSTRUCTION)==0) /*read, 0=byte, 1= word*/
	...
	else if(GetR_W(CURRENT_LATCHES.MICROINSTRUCTION) ==1) /*write*/
			{
				...
				else if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION)==0) /* data.size = 0 = byte*/
				{
					...
					{
					MEMORY[CURRENT_LATCHES.MAR>>1][1]= CURRENT_LATCHES.MDR&0x00FF; /*clear it out */
					}
...
/*CYCLE_COUNT = 300, then interrupt*/

int SR1Temp, SR2Temp, ALUOUT, MARMUXTEMP, SHFTemp, MDROUT, SPMUXOUT;
int BUSOUT;
int ADD1,ADD2;
int A, B;
int VECTOROUT, VECTORMUXOUT, PSRMUXOUT;
void eval_bus_drivers() {
  /* 
   ...
   *		 Gate_SHF,
   *		 Gate_MDR.
   ADD IN GateVector GatePC-2 GatePSR GateSP
   LD.Priv	LD.SavedSSP	LD.SavedUSP	LD.Vector	GateVector	GatePC-2	GatePSR	GateSP	SPMUX	SPMUX	VectorMUX	PSRMUX	Set.Priv

   */    
if(GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION)==1) /*SR1MUX, 0 = 11:9 1 = 8:6, 2 = SP Source R6*/
{
...
{
	SR1Temp = Low16bits(CURRENT_LATCHES.REGS[6]);
}
...
{
	B= (SEX((CURRENT_LATCHES.IR&0x001F),5));
...
ALUOUT = ALUOUT&0xFFFF; /*do i need to sign extend this? */
   ...
   SHFTemp = Low16bits(Low16bits(SR1Temp) >> amount4);
   break;
   case 2: /*SR15 should be moved in
  ...
  /* printf("\n mdrout = mdr = %d \n", CURRENT_LATCHES.MDR);*/
   ...
   else if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION) ==0)
   {
		if(CURRENT_LATCHES.MAR&0x0001 ==1)
		{
			MDROUT = CURRENT_LATCHES.MDR&0x00FF;
		}
		else if(CURRENT_LATCHES.MAR&0x0001 ==0)
		{
             MDROUT = (CURRENT_LATCHES.MDR&0xFF00)>>8;
		}
   }
   
	if(GetVECTORMUX(CURRENT_LATCHES.MICROINSTRUCTION)) /*Vector mux, INTV=0, EXCV= 1*/
	{
		VECTORMUXOUT = CURRENT_LATCHES.EXCV;
	}
	else
	{
...
void drive_bus() {
   ...
   * tristate drivers. 
	...
	GATEPC_2,
	GATEPSR,
	GATESP are added, 9 total gates now
   */  	     
...
if(GetLD_MDR(CURRENT_LATCHES.MICROINSTRUCTION)) /*questions about memory, looks like MDROUT*/
{
	...
	{
		NEXT_LATCHES.MDR = Low16bits(SEX(MEMORY[CURRENT_LATCHES.MAR >> 1][1],8));
	}
	else if ((CURRENT_LATCHES.MAR&0x0001) ==0)
	{
		NEXT_LATCHES.MDR = Low16bits(SEX(MEMORY[CURRENT_LATCHES.MAR >> 1][0],8));
	}
	}
	}
}
